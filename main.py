...
root = Tk()
root.title('Udemy Downloader 2022  V.1.02 | supported by feat-shaka Branch')
...
def my_program():
    ...
    try:
        ...
        print('Your Connection is failed ~~ Please try and run again ~')
        main()
    ...
    def _get(self, url):
        ...
        if alert_nan_trager > 0:
            if str(session.status_code) == "404":
                alerts_showerror(
                    "Please Fill The URL of Your Selected Course Correctly! You Probably Need to Check it Again!  We tried " + str(
                        alert_nan_trager) + " Times to Fitch Your Course And It's Failed Cause Wrong URL Given !!\n\nYou Can Define Number of Retrying Attempts from Starter Screen")
            elif str(session.status_code) == "403":
                alerts_showerror(
                    "Please Fill Your Access Token Field Right !!\nYou Probably Need to Check it Again!  We Tried " + str(
                        alert_nan_trager) + " Times and it's Failed Cause Wrong Access Token Given !!\n\nYou Can Define Number of Retrying Attempts from Starter Screen")
        # else:
